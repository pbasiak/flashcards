...
function FlashCardForm({ handleSubmit, handleChange, values, errors, handleCancel, isSubmitDisabled, submitText }) {
    ...
    const tagsSelectItems = tagsValues.map(tag => <MenuItem key={`${tag.id}_${tag.name}`} value={tag}>{tag.name}</MenuItem>);
    const decksSelectItems = decksValues.map(deck => <MenuItem key={`${deck.id}_${deck.Title}`} value={deck}>{deck.Title}</MenuItem>);
    ...
    return (
        <form onSubmit={handleSubmit} className={classes.form}>
            ...
            <Box display="flex" mt="40px" justifyContent="flex-end">
                <Box mr="16px"><Button variant="outlined" color="primary" onClick={handleCancel}>Cancel</Button></Box>
                <Button disabled={isSubmitDisabled} variant="contained" color="primary" type="submit">{submitText}</Button>
            </Box>
        </form>
    );
}
